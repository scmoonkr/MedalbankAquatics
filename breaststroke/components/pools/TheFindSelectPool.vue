// '/components/TheFindSelectPool.vue'
<template>
  <div v-if='isSelected === false' class="pool-search-container">
    <div class="search-input-container">
      <input type="text" v-model="searchQuery" class="pool-input" placeholder="수영장 이름 검색" @input="handleSearch" />
    </div>
    ...
    <div v-else-if="searchResults.length > 0" class="search-results">
      <button v-for="pool in searchResults" :key="pool.nameID" class="pool-result-button" @click="selectPool(pool)">
        {{ pool.name }}
      </button>
    </div>
    ...
    <p class="hint-text">수영장 이름을 선택해주세요. 검색되지 않는 수영장은 메일로만 처리 바랍니다.</p>
  </div>
  <!-- <div v-if="selectedPool" class="selected-pool">
    <p>선택된 수영장: <strong>{{ selectedPool.name }}</strong></p>
  </div> -->
...
if (props.modelValue && props.modelValue.name) {
  ...
  selectedPool.value = props.modelValue;
}
...
const handleSearch = debounce(async () => {
  ...
  try {
    ...
    // 실제 API 호출 (실제 구현 필요)
    await poolStore.fetchPoolNames(searchQuery.value);
    // const response = await fetch(`/api/pools/search?query=${searchQuery.value}`);
