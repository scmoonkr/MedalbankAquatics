<template>
  ...
  <DataTable :columns="columns" :data="competitions" :pagination="true" :totalPage=0 :currentPage=0 :itemsPerPage=0 :search="false" :isLoading="false" :serverSide="true" alternateRowColors="#FBE2D5" alternateColumnColors="#DAE9F8" @click-cell="clickTableCell">
    <template #column-discipline="{ value, row }">
      <CellDisciplineSimple :text1="`${getGenderByEng(value)} ${getStyleKorByEng((row as any).style)} ${(row as any).distance} ${(row as any).course}`" />
    </template>
    ...
    <!-- <template #column-name="{ value, row }">
      <CellAthleteText :image="(row as any).thumbnail" :name="(row as any).name" :id="(row as any).athleteID" :text="(row as any).ageGroup" @image-error="handleImageError" />
    </template> -->


    <template #column-photo="{ row }">
      <CellAthletePhoto :image="getImageURL(row.thumbnail)" @image-error="handleImageError" :unregistered="!row.athleteID" />
    </template>

    <template #column-name="{ row }">
      <CellAthleteText :name="row.name" :id="row.athleteID" :text="row.ageGroup" :unregistered="!row.athleteID" />
    </template>

    <template #column-competitionName="{ value, row }">
      <CellTextTextText :text1="(row as any).datetime ?? ''" :text2="(row as any).competitionName ?? ''" :text3="(row as any).pool ?? ''" :unregistered="!row.athleteID" />
    </template>
  </DataTable>
...
const props = defineProps({
  competition: {
    type: Object as () => CompetitionModel,
    required: false,
    default: null
  },
});
...
const competitions = computed(() => {
  ...
  // 실제 competition 객체의 구조에 따라 다음 경로를 수정해야 할 수 있습니다
  return sortTimes(props.competition.extraInfo?.bestStems || []);
  // return props.competition.extraInfo?.bestStems || [];
});
