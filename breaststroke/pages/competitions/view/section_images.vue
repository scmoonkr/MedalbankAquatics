<template>
  <div class="image-cards-container">
    <TheImageCard_0 :competition="competition" :medals="medals" :date="dateStr" width="normal" :card-height="height"
      :background-image="selectedImage" :text-color="selectedTextColor" />

    <TheImageCard_1 :competition="competition" :medals="medals" :date="dateStr" width="normal" :card-height="height"
      :background-image="selectedImage" :text-color="selectedTextColor" />

    <TheImageCard_2 :competition="competition" :medals="medals" :date="dateStr" width="normal" :card-height="height"
      :background-image="selectedImage" :text-color="selectedTextColor" />

    <TheImageCard_0 :competition="competition" :medals="medals" :date="dateStr" width="narrow" :card-height="height"
      :background-image="selectedImage" :text-color="selectedTextColor" />

    <TheImageCard_1 :competition="competition" :medals="medals" :date="dateStr" width="narrow" :card-height="height"
      :background-image="selectedImage" :text-color="selectedTextColor" />

    <TheImageCard_2 :competition="competition" :medals="medals" :date="dateStr" width="narrow" :card-height="height"
      :background-image="selectedImage" :text-color="selectedTextColor" />

    <TheImageCard_3 :competition="competition" :medals="medals" :date="dateStr" width="normal" :card-height="height"
      :background-image="selectedImage" :text-color="selectedTextColor" />

    <TheImageCard_0 :competition="competition" :medals="medals" :date="dateStr" width="wide" :card-height="height"
      :background-image="selectedImage" :text-color="selectedTextColor" />
  </div>
...
const defaultImage = ""; //"https://i.ibb.co/BK3ftvbn/competition-23-featured.jpg";
...
// competition 객체 변경 감지 및 이미지 업데이트
watch(() => props.competition, (newCompetition) => {
  dateStr.value = getDateFormat("2025-04-01") + " 기준";

  // medals 전체 합계 계산
  medals.value = newCompetition.eventStatistics?.reduce((acc: any, event: any) => {
  ...
  if (newCompetition) {
    let featured = newCompetition.featured ?? "";
    ...
    // if (newCompetition.featured) {
    //   selectedImage.value = newCompetition.featured;
    //   originalImage.value = newCompetition.featured; // 원본 이미지 저장
    // }
  }
}, { immediate: true });
...
onMounted(() => {
  let featured = props.competition.featured ?? "";
  featured = featured.includes("http") ? featured : apiBase + featured;
  selectedImage.value = featured;
...
async function saveImageToServer() {
  if (!props.competition || !props.competition.competitionID) {
  ...
  try {
    ...
    // competitionID: number, imageURL: string, userID: number
