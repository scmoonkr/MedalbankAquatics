<template>
  ...
  <DataTable
    ...
    :isLoading="false"
    :serverSide="true"
    alternateRowColors="#FBE2D5"
    alternateColumnColors="#DAE9F8"
    ...
    <template #column-name="{ row }">
      <CellAthleteText :image="getImageURL((row as any).thumbnail)" :name="(row as any).name" :id="(row as any).athleteID" :text="(row as any).ageGroup" @image-error="handleImageError" :unregistered="!(row as any).athleteID&&(row as any).isMasters != false" />
    </template>
...
const competitions = computed(() => {
  ...
  const times = sortTimes(props.competition.extraInfo?.bestTimes || []);
  return times;
});
