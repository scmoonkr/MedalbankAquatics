...
<template>
  <div class="datatable-desktop">
    ...
    <div class="relative">
      ...
      <!-- 카테고리 - 이미지 상단 좌측 -->
      <div v-if="item.category.length > 0" class="absolute top-2 left-2 bg-blue-500 text-white text-xs px-2 py-1 rounded-sm shadow-md">
        {{ item.category ?? '' }}
      </div>

      <!-- 추천수 - 이미지 상단 우측 -->
      <div v-if="item.type.length > 0"
        class="absolute top-2 right-2 bg-red-500 text-white text-xs px-2 py-1 rounded-sm shadow-md flex items-center">
        <span class="font-bold mr-1">♥</span> {{ item.type??'' }}
      </div>

      
      <div v-if="item.brand.length > 0"
    ...
    <div class="p-3 pt-2 flex-1 bg-gray-100 flex flex-col content-info-container">
      ...
      <div class="title-container min-h-[3.5rem] flex items-center my-2">
        <h3 v-if="item.title" class="text-lg font-semibold text-gray-900 line-clamp-2">
