<template>
  ...
  <DataTable :columns="columns" :data="props.times" :pagination="true" :totalPage=0 :currentPage=0 :itemsPerPage=0 :search="false" :isLoading="false" :serverSide="true" alternateRowColors="#FBE2D5" alternateColumnColors="#DAE9F8" @click-cell="clickTableCell">
    <template #column-gender="{ value, row }">
      <CellText :text="`${getGenderByEng(value)} ${getStyleKorByEng((row as any).style)} ${(row as any).distance} ${(row as any).course}`" />
    </template>

    <template #column-time="{ row }">
      <CellTimeDate :time="(row as any).time ?? ''" :date="(row as any).datetime ?? ''" />
    </template>

    <template #column-photo="{ row }">
      <CellAthletePhoto :image="getImageURL((row as any).thumbnail)" :unregistered="!(row as any).athleteID" @image-error="handleImageError" />
    </template>

    <template #column-name="{ row }">
      <CellAthleteText :name="(row as any).name" :id="(row as any).athleteID" :text="(row as any).ageGroup" :unregistered="!(row as any).athleteID" @image-error="handleImageError" />
    </template>

    <template #column-competitionName="{ row }">
...
//   // 실제 pool 객체의 구조에 따라 다음 경로를 수정해야 할 수 있습니다
//   return props.pool.extraInfo?.bestTime || [];
// });
...
// DataTable 설정
const columns = [
  { key: 'gender', label: '종목', sortable: false },
  { key: 'time', label: '결과', sortable: false },
  { key: 'photo', label: '', sortable: false },
  { key: 'name', label: '이름', sortable: false },
  { key: 'competitionName', label: '대회', sortable: false },
];


// 테이블 셀 클릭 처리
const clickTableCell = (item: {}, column: string) => {
  switch (column) {
