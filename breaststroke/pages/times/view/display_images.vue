<template>
  <div v-if="time?.timeID > 0" class="image-cards-container">
    <TheImageCard_0 :time="time" :medals="medals" :date="dateStr" width="normal" :card-height="height"
      :background-image="selectedImage" :text-color="selectedTextColor" />

    <TheImageCard_1 :time="time" :medals="medals" :date="dateStr" width="normal" :card-height="height"
      :background-image="selectedImage" :text-color="selectedTextColor" />

    <TheImageCard_2 :time="time" :medals="medals" :date="dateStr" width="normal" :card-height="height"
      :background-image="selectedImage" :text-color="selectedTextColor" />

    <TheImageCard_0 :time="time" :medals="medals" :date="dateStr" width="narrow" :card-height="height"
      :background-image="selectedImage" :text-color="selectedTextColor" />

    <TheImageCard_1 :time="time" :medals="medals" :date="dateStr" width="narrow" :card-height="height"
      :background-image="selectedImage" :text-color="selectedTextColor" />

    <TheImageCard_2 :time="time" :medals="medals" :date="dateStr" width="narrow" :card-height="height"
      :background-image="selectedImage" :text-color="selectedTextColor" />

    <TheImageCard_3 :time="time" :medals="medals" :date="dateStr" width="normal" :card-height="height"
      :background-image="selectedImage" :text-color="selectedTextColor" />

    <TheImageCard_0 :time="time" :medals="medals" :date="dateStr" width="wide" :card-height="height"
      :background-image="selectedImage" :text-color="selectedTextColor" />
  </div>

  <div v-if="athleteID != undefined && time != undefined && athleteID == time.athleteID" class="image-picker-container">
    <!-- Image picker input -->
...
const defaultImage = ""; //"https://i.ibb.co/BK3ftvbn/time-23-featured.jpg";
...
const isUploading = computed(() => timeStore.isImageUploading);
...
// time 객체 변경 감지 및 이미지 업데이트
watch(() => props.time, (newTime) => {
  dateStr.value = getDateFormat("2025-04-01") + " 기준";
  selectedImage.value = '';
  originalImage.value = ''; // 원본 이미지 저장
  if (newTime == undefined) return { gold: 0, silver: 0, bronze: 0, };


  // medals 전체 합계 계산
  medals.value = newTime.eventStatistics?.reduce((acc: any, event: any) => {
  ...
  if (newTime) {
    newTime.featured = newTime.featured || "";
    const featured = newTime.featured.includes("http") ? newTime.featured : apiBase + newTime.featured;
    selectedImage.value = featured;
    originalImage.value = featured; // 원본 이미지 저장

    // if (newTime.featured) {
    //   selectedImage.value = newTime.featured;
    //   originalImage.value = newTime.featured; // 원본 이미지 저장
    // }
  }
}, { immediate: true });

// 스토어 에러 상태 감시
watch(() => timeStore.imageUploadError, (newError) => {
  if (newError) {
...
onMounted(() => {
  ...
  let featured = props.time.featured || "";
...
async function saveImageToServer() {
  if (!props.time || !props.time.timeID) {
  ...
  try {
    ...
    // timeID: number, imageURL: string, userID: number
