<template>
  ...
  <DataTable :columns="columns" :data="props.times" :pagination="true" :totalPage=0 :currentPage=0 :itemsPerPage=0 :search="false" :isLoading="false" :serverSide="true" alternateRowColors="#FBE2D5" alternateColumnColors="#DAE9F8" @click-cell="clickTableCell">
    ...
    <template #column-name="{ value, row }">
      <CellAthleteText :image="getImageURL((row as any).thumbnail || '/images/not_found.jpg')" :name="(row as any).name" :id="(row as any).athleteID" :text="(row as any).ageGroup" />
    </template>

    <template #column-competitionName="{ value, row }">
      <CellCompetitionText :competition="(row as any).competitionName ?? ''" :text="(row as any).pool ?? ''" />
    </template>
