...
// editingItem 타입 수정
interface EditablePool extends PoolModel {
...
const props = defineProps({
  pools: {
    type: Array as () => PoolModel[],
    default: () => [] as PoolModel[],
  },
  totalPages: {
    type: Number,
    default: 1,
...
//===================================
// modelValue 변경 감지
watch(() => props.pools, (newValue) => {
  tableData.value = newValue;
}, { deep: true });
// const tableData = computed(() => {
//   // return poolStore.filteredTimeList;
//   return props.pools.map((data:PoolModel) => {
//     // checked 속성 추가
//     return {
//       ...data,
//       checked: selectedItems.value.includes(data.poolID)
//     };
//   });
// });
...
const sortColumnChanged = async (column: string) => {
  poolStore.filter.sortField = column as PoolSortField;
  poolStore.filter.sortDirection = poolStore.filter.sortDirection === SortDirection.ASC ? SortDirection.DESC : SortDirection.ASC;
  await poolStore.fetchPoolList(1, poolStore.pagination.rowsPerPage);
};

const clickTableCell = async (item: PoolModel, column: string) => {
...
const toggleItemChecked = (poolID: number, checked: boolean) => {
  const item = tableData.value.find(item => (item as any).poolID === poolID);
  if (item) item.checked = checked;
};
// 삭제 관련 함수
const confirmDelete = (poolID: any) => {
  itemToDelete.value = poolID;
  showDeleteModal.value = true;
};

const confirmDeleteItem = () => {
  if (itemToDelete.value != undefined) {
    tableData.value = tableData.value.filter(item => (item as any).poolID != (itemToDelete.value as any).poolID);
  }
  // close Modal
...
const saveEditedItem = () => {
  ...
  //-------------------------------
  const index = tableData.value.findIndex(item => (item as any).poolID === (editingItem as any).value.poolID);
  if (index !== -1) {
