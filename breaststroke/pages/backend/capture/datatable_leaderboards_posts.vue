<template>
  <!-- DataTable 컴포넌트 -->
  <DataTablePosts :fixed="true" :title="props.title" :subtitle="props.subtitle||''" :date="(props.datetime || '')" :columns="columns" :data="tableData" :pagination="false" :totalPage="props.totalPages" :currentPage="currentPage" :itemsPerPage="rowsPerPage" :search="false" :isLoading="isLoading" :serverSide="true" :alternateRowColors="true" @sort-column="sortColumnChanged" @click-cell="clickTableCell" @page-change="pageChanged">
        ...
        <div class="flex flex-col">
          <div :class="(!(slotProps.row as any).athleteID || !(slotProps.row as any).athleteID) ? 'text-white/10' : 'text-white'" class="text-[28px] leading-[40px]">
            <template v-if="!(slotProps.row as any).athleteID">
              {{ (slotProps.row as any).nameHide ?? '미등록' }}
            </template>
            <template v-else>
          ...
          <div v-if="true || (slotProps.row as any).athleteID" class="text-[16px] text-white/50">
