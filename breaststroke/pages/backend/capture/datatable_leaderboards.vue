...
<template>
  ...
  <DataTable
    ...
    <template #column-time="slotProps">
      <CellTimeDate :time="(slotProps.row as any).time ?? ''" :date="(slotProps.row as any).datetime ?? ''" />
    </template>

    <template #column-name="slotProps">
      <CellAthleteText :image="getImageURL((slotProps.row as any).thumbnail ?? '')" :name="(slotProps.row as any).name ?? ''" :id="(slotProps.row as any).athleteID ?? 0" :text="(slotProps.row as any).team ?? ''" :unregistered="!(slotProps.row as any).athleteID" />
    </template>
...
const columns = [
  ...
  { key: 'time', label: '기록', sortable: true },
  { key: 'name', label: '선수', sortable: true },
