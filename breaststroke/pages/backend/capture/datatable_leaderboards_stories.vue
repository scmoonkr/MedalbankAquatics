// datatable_leaderboards_stories.vue
<template>
  <!-- DataTable 컴포넌트 -->
  <DataTableStories :fixed="true" :title="props.title" :subtitle="props.subtitle||''" :date="(props.datetime || '')" :columns="columns" :data="tableData" :pagination="false" :itemsPerPage="rowsPerPage" :search="false" :isLoading="isLoading" :serverSide="true" :alternateRowColors="true" @sort-column="sortColumnChanged" @click-cell="clickTableCell" @page-change="pageChanged">
    <!-- 시간 셀 -->
    ...
    <template #column-name="slotProps">
      <div class="flex items-center text-left gap-[64px]">
        ...
        <template v-else>
          <div class="object-cover w-[152px] h-[152px]" :class="(!(slotProps.row as any).athleteID) ? 'bg-[#0f172a]/10' : 'bg-white/20'"></div>
        </template>
