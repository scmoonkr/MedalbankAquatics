...
import { StemSortField, type Stem } from '~/types/stems';
...
// editingItem 타입 수정
interface EditableStem extends StemModel {
...
const props = defineProps({
  stems: {
    type: Array as () => StemModel[],
    default: () => [] as StemModel[],
  },
  totalPages: {
    type: Number,
    default: 1,
...
//===================================
// modelValue 변경 감지
watch(() => props.stems, (newValue) => {
  tableData.value = newValue;
}, { deep: true });
// const tableData = computed(() => {
//   // return stemStore.filteredTimeList;
//   return props.stems.map((data:StemModel) => {
//     // checked 속성 추가
//     return {
//       ...data,
//       checked: selectedItems.value.includes(data.stemID)
//     };
//   });
// });
...
const sortColumnChanged = async (column: string) => {
  stemStore.filter.sortField = column as StemSortField;
  stemStore.filter.sortDirection = stemStore.filter.sortDirection === SortDirection.ASC ? SortDirection.DESC : SortDirection.ASC;
  await stemStore.fetchStemList(1, stemStore.pagination.rowsPerPage);
};

const clickTableCell = async (item: StemModel, column: string) => {
  // router.push(`/stem/${item.stemID}`);
...
// 삭제 관련 함수
const confirmDelete = (stemID: any) => {
  itemToDelete.value = stemID;
  showDeleteModal.value = true;
};

const confirmDeleteItem = () => {
  if (itemToDelete.value != undefined) {
    tableData.value = tableData.value.filter(item => (item as any).stemID != (itemToDelete.value as any).stemID);
  }
  // close Modal
...
const saveEditedItem = () => {
  ...
  //-------------------------------
  const index = tableData.value.findIndex(item => (item as any).stemID === (editingItem as any).value.stemID);
  if (index !== -1) {
