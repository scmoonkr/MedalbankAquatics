...
class CompetitionModel {
	...
	static create = async () => {
		...
		await mongodb.createCollectionNindex(mongoCFG.Breaststroke.athletes, indexes);
	}
	static createView = async () => {
		let returnObj = { message: "", };
		try {
			const viewName = mongoCFG.Breaststroke.competitionsView;
			const viewON = mongoCFG.Breaststroke.competitions;
			const pipeline = [
				{ $lookup: {
						from				: mongoCFG.Breaststroke.competitionsStatistics,
	...
	static async list(query, body={}) {
		...
		//----------------------------------------------------------------
		const result = await mongodb.find(mongoCFG.Breaststroke.competitions, context)
		...
		result.data.forEach(competition => {
			competition.fullname = competition.fullname ?? competition.competitionName;
			competition.dateStart = competition.dateStart ?? competition.datetime;
	...
	static async viewNew(body) {
		...
		*/
		...
		//-----> select competitions
		const result = await mongodb.aggregate(mongoCFG.Breaststroke.competitions, aggregate);
		...
		if (data.stemID) {
			...
			const resultStems = await mongodb.find(mongoCFG.Breaststroke.competitions, context);
	...
	static async detail(body) {
		...
		//----------------------------------------------------------------
		const returnObj = await mongodb.findOne(mongoCFG.Breaststroke.competitions, { competitionID: Number(body.competitionID) }, { _id:0,});
	...
	static async brief(body) {
		...
		const returnObj = await mongodb.findOne(
																			mongoCFG.Breaststroke.competitions,
	...
	*/
	static saveWithImage = async (body) => {
		...
		if (!body.competitionID || body.competitionID == 0) {
				...
				value.competitionID = await mongodb.max(mongoCFG.Breaststroke.competitions, "competitionID", {}) + 1;
		} else {
		...
		// --------------------------------------------------------------
		await mongodb.updateOne(mongoCFG.Breaststroke.competitions, query, value);
...
static async stemNames(body) {
	...
	//-----> select competitions
	//----------------------------------------------------------------
	const result = await mongodb.find(mongoCFG.Breaststroke.stems, context);
...
static async saveStem(body) {
	...
	//-----> select competitions
	//----------------------------------------------------------------
	const result = await mongodb.findOne(mongoCFG.Breaststroke.stems, query, { _id:0, });
	...
	value.stemID = await mongodb.max(mongoCFG.Breaststroke.stems, "stemID", {});
	await mongodb.insertOne(mongoCFG.Breaststroke.stems, value);
	...
	static async view(body) {
		...
		*/
		...
		const aggregate = [
			{ $match: query },
			{ $lookup: {
					from				: mongoCFG.Breaststroke.pools,
					localField	: "poolID",
					foreignField: "poolID",
					as					: "pools"
				}
			...
			{
				$lookup: {
					from: mongoCFG.Breaststroke.competitions,
					let: { stemID: "$stemID" },
					pipeline: [
						{
							$match: {
...
console.log("query:", query);
	...
	static getUpcommingCompetitions = (async (body) => {
	 ...
	 static async buildStatistics(competitionID) {
		...
		//  console.log("---->", times.data);
		 ...
		 await mongodb.updateOne(mongoCFG.Breaststroke.competitionsStatistics, query, statistics);
		 return statistics;
	 }
  ...
  //####################################################################
	...
	static async names(body) {
		const result = await mongodb.distinct(mongoCFG.Breaststroke.times, "competitionID");
		...
		//-----> select competitions
		//----------------------------------------------------------------
		return await mongodb.find(mongoCFG.Breaststroke.competitions, context);
		//----------------------------------------------------------------
	}
	...
	static async years(body) {
		//----------------------------------------------------------------
		const result = await mongodb.distinct(mongoCFG.Breaststroke.competitions, "year",{ fullname: { $exists: true }, year:{$gt:0} });
	...
	static async ageGroup(body) {
		...
		//------------------------------------------------------------------
		let result = await mongodb.findOne(mongoCFG.Breaststroke.competitions, query, { _id:0, ageGroup:1 }  );
		//-----> check undefined
		if (!result.data.ageGroup || result.data.ageGroup.length == 0) {
			result = await mongodb.distinct(mongoCFG.Breaststroke.times, "ageGroup", query );
			await mongodb.updateOne(mongoCFG.Breaststroke.competitions, query, { ageGroup: result.data } );
		} else {
	...
	console.log("query=", query);
		const returnObj = await mongodb.find(mongoCFG.Breaststroke.competitions, context);
	...
	static async times(body) {
		...
		const res = await mongodb.distinct(mongoCFG.Breaststroke.times, "competitionID");
		const context =  {
	...
	static async insert(body) {
		...
		//----------------------------------------------------------------
		value.competitionID = await mongodb.max(mongoCFG.Breaststroke.competitions, "competitionID", {}); // query, db
		console.log(">>>>>>>>>>>>>> competitions.insert.value=", value);
		 await mongodb.insertOne(mongoCFG.Breaststroke.competitions, value);
		 return { message: '', data: { competitionID: value.competitionID } };
		//----------------------------------------------------------------
	}
  ...
  static async updateDelete(competitionID, userID) {
    const query = { competitionID: Number(competitionID), userID: Number(userID) };
      ...
      deleted : new Date(),
		...
		//----------------------------------------------------------------
		returnObj = await mongodb.updateOne(mongoCFG.Breaststroke.competitions, query, value);
		//----------------------------------------------------------------
	...
	static async delete(competitionID) {
		try {
			const query = { competitionID: Number(competitionID) };
			//----------------------------------------------------------------
			return await mongodb.deleteOne(mongoCFG.Breaststroke.competitions, query);
			//----------------------------------------------------------------
		} catch (e) {
