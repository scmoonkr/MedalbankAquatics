...
exports.ItemController = {
  ...
  portfolio: async (req, res, next) => {
    try {
        console.log("controller.items.portfolio.params.", req.params);
        const result = await Services.portfolio(req.params);
        res.json(result);
    } catch (err) {
        console.log("controller.items.portfolio.catch.", err);
        next(err);
    }
  },
  collections: async (req, res, next) => {
    try {
        console.log("controller.items.collections.params.", req.body);
        const result = await Services.collections(req.body);
        res.json(result);
    } catch (err) {
        console.log("controller.items.collections.catch.", err);
        next(err);
    }
  },
