...
class TimeLibrary {
	...
	getCompetitionsTeamsPools = (times) => {
		...
		const restructuredTimes = times.map(time => {
			...
			if (time.poolID) {
				// const pool = memoryDB.getPoolName[time.poolID];
				// console.log("pool=", pool, time.poolID);
				pools.set(time.poolID, { poolID: time.poolID, name: memoryDB.poolOBJ[time.poolID].fullname ?? "" });
			}
			...
			if (time.teamID) {
				...
				teams.set(time.teamID, { teamID: time.teamID, name:  memoryDB.teamOBJ[time.teamID].name ??"" });
			}
			
			// Collect unique competitions
			if (time.competitionID) {
				// const competition = memoryDB.getCompetitionName[time.competitionID];
				// console.log(competition);
				competitions.set(time.competitionID, { 
					competitionID: time.competitionID, 
					name:  memoryDB.competitionOBJ[time.competitionID].fullname, // time.competitionName,
	...
	getCompetitionsTeamsPoolsOld = (times) => {
		...
		try {
			...
			if (competitionIDs && competitionIDs[0] != undefined) {
				result.competitions = competitionIDs.reduce((arr, id) => {
			...
			if (teamIDs && teamIDs[0] != undefined)
				result.teams = teamIDs.reduce((arr, id) => {
			...
			if (poolIDs && poolIDs[0] != undefined)
				result.pools = poolIDs.reduce((arr, id) => {
    ...
    const sorted = Object.entries(counts).filter(data => data.teamID != 'undefined')
																				 .map(([data, count]) => ({
																						teamID: Number(data),
																						name: memoryDB.getTeamName(Number(data)),
																						count,
																					}))
																					.sort((a,b) => b.count-a.count)
																					.slice(0, count);
...
console.log("best=", best[0]);
	...
	***********************************************************************************/
	...
	makeStatisticsOLD(times) {
		...
		statistics.teams        = this.getTeamsByCount(times);
		...
		statistics.firstEvent    = eventTimes.length == 0 ? {} : this.findFirstTime(eventTimes);
		statistics.latestEvent   = eventTimes.length == 0 ? {} : this.findLatestTime(eventTimes);
		// statistics.bestEvent     = eventTimes.length == 0 ? {} : this.findBestTimeJSON(eventTimes);
		statistics.bestEvent     = eventTimes.length == 0 ? {} : this.calculateBestTimeWithSeason(eventTimes);
		...
		statistics.firstTime     = this.findFirstTime(timeTimes);
		statistics.latestTime    = this.findLatestTime(timeTimes);
		// statistics.bestTime      = timeTimes.length == 0 ? {} : this.findBestTimeJSON(timeTimes);
		statistics.bestTime      = timeTimes.length == 0 ? {} : this.calculateBestTimeWithSeason(timeTimes);
		statistics.timekeeper    = timeTimes.length == 0 ? [] : this.getTimekeepersByCount(timeTimes);
