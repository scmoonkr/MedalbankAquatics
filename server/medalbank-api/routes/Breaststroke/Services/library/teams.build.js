...
exports.buildTeamsStatistics = async (query) => {
	...
	let result = await mongodb.find(mongoCFG.Breaststroke.times, context);
	...
	result = await mongodb.deleteMany(mongoCFG.Breaststroke.teamStatics, { teamID: { $in: teamIDs } });
	console.log("delete=", result);
	result = await mongodb.insertMany(mongoCFG.Breaststroke.teamStatics, teams );
...
exports.calculateTeamStatistics = async (timeArr) => {
	...
	// const config = await mongodb.findOne(mongoCFG.Breaststroke.config, { type: "teamPoints" }, { _id:0, type:0, } );
