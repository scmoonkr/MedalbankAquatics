...
const utilTimes    = require("../../../Class/utilTimes");
...
exports.checkDuplicate = (times) => {
	...
	const timeOBJ = {};
	for (const time of times) {
		// const key = `${time.competitionID}-${time.gender}-${time.ageGroup}-${time.discipline}-${time.distance}-${time.name}-${time.team}-${time.time}`;
...
console.log("filename=", filename);
	const excelTimes = await excel.read_excel_org(`${path}/${filename}`);
console.log("excelTimes.", excelTimes.length, );
	const timeArr = formatTimes(excelTimes);

	const times = customizingTimesORG(timeArr, competition);
	console.log("-----> readTimesExcel", times.slice(0, 1));
	return times;
}
    ...
    for (const row of timeArr) {
      ...
      if (Object.keys(dsp).length > 2) {
        ...
        continue;
			...
			if (time.discipline && time.discipline.includes("계영") && !time.name && time.team) time.name = time.team;

			//-----> check timesHeader row
      if (_headerTime.includes(time.time) || time.name && _headerName.includes(time.name.replace(/ /gi, ''))) continue;
  
      //-----> 선수명, 팀명이 없으면
			if (discipline && discipline.discipline && discipline.discipline.includes("계영") && !time.name && time.team) {
				time.name = time.team
			}
      ...
      time.isMasters			= time.isMasters || time.masters || true;
		...
		// console.log("time=", time);
			...
			if (time.discipline && !Util.disciplineTable.find(discipline => time.discipline == discipline.discipline)) {
				const ck = Util.disciplineTable.find(discipline => time.discipline.includes(discipline.label));
				if (ck) {
					if (!time.distance) {
						const distance = time.discipline.replace(ck.label, '').replace('m', 'M').trim();
      ...
      //-----> get discipline
			if (!time.distance) {
				const arr = time.discipline.split(' ');
				if (arr.length > 1) {
					arr[0] = arr[0].trim();
					if (_distances.includes(arr[0])) { time.distance = arr[0]; time.discipline = arr[1].trim(); }
					arr[1] = arr[1].trim();
					if (_distances.includes(arr[1])) { time.distance = arr[1]; time.discipline = arr[0].trim(); }
				}
			}
      time.discipline = Util.getDiscipline(time.discipline);
      if (time.discipline.includes("혼성")) {
        time.gender = "mixed";
        time.discipline = time.discipline.replace("혼성", "");
      }
      ...
      //-----> check 단체전
      time.individual = !time.discipline.includes("Relay");
      ...
      if (!message.discipline.includes(time.discipline) ) 								message.discipline.push(time.discipline);
  ...
  console.log("++++++++", filename);
	...
	XlsxPopulate.fromBlankAsync().then(async (workbook) => {
		...
		for (let col=0; col < headers.length; col++) {
			...
			sheet.cell(`${cellAlpha[col]}${rowno}`).value(headers[col].label).discipline(style);
		...
		for (let row=0; row < rows.length; row++) {
			...
			for (let col=0; col < headers.length; col++) {
				...
				if ("times,timeORG,name,team,discipline,distance,gender,teamID".includes(headers[col].column) || headers[col].column == "ageGroup") {
					if (!rows[row][headers[col].column]) {
				...
				// 	}
				// }
				sheet.cell(`${cellAlpha[col]}${rowno}`).discipline(style);
