...
const _disciplinesKorean 	= [ "접영", "배영", "평영", "자유형", "개인혼영", "혼계영", "혼성계영", "혼성혼계영", "계영",];
...
exports.getDiscipline = (label => {
	label = label.replace(/ /g, '');
	const ck = this.disciplineTable.find(discipline => discipline.label==label);
	return !ck ? label : ck.discipline;
})
...
const timesHeader = [
	...
	{ label: "영법", column: "discipline", type: "s", width: 50, },
	...
	// cid	masters	adult	gender	ageGroup	discipline	distance	round	name	team	rank	times
...
exports.headers = [
	...
	{ label:"Heat,HEAT,heat,경기번호", name: 'heat' },
...
exports.splitDiscipline = (ageGroup) => {
	...
	// 2. discipline: "접영핀,배영핀,평영핀,자유형핀,개인혼영,혼성계영,혼계영,계영,혼성혼계영,핀계영"
	...
	for (let key of arr) {
		...
		let find = _disciplinesKorean.find(el => key.includes(el));
		if (find) {
	...
	if (!discipline.gender && discipline.discipline && discipline.discipline.includes("혼성")) discipline.gender = "mixed";
...
exports.checkDiscipline = (time) => {
	...
	if (!time.heat) return discipline;

	if (!time.heat && time.discipline && time.discipline.length > 6) time.heat = time.discipline;
	time.heat = time.heat ? time.heat.toString() : "";
	if (time.heat.length < 5) return discipline;

	let ageGroup = "";

	if (time.discipline && time.discipline.length > 6) {
		ageGroup = time.discipline;	
	} else if (time.heat && time.heat.length > 5) {
		ageGroup = time.heat;
...
exports.mergeTimes2Athletes = (timeArr, athleteID) => {
	...
	for (const time of timeArr) {
		try {
			...
			athletes[discipline].times.push({ discipline: time.discipline + "-" + time.distance, timeID: time.timeID });
		} catch (e) {
	...
	Object.keys(athletes).forEach(key => {
		...
		if (athlete.times.length == 2) {
			if (athlete.times[0].discipline == athlete.times[1].discipline) {
				// console.log("same discipline =====>", athlete);
				disciplines++;
				check = false;
			}
		}
	...
	return { athletes: athleteArr, times2, disciplines, athleteID };
}
...
exports.customCompleteTimes = (times, _competition) => {
	...
	times.forEach(time => {
		...
		try {
			...
			time.discipline 		= time.discipline 		|| oldTime.discipline;
...
async function write_excel_sheet_new(filename, rows, header=timesHeader) {
	...
	XlsxPopulate.fromBlankAsync().then(async (workbook) => {
		...
		for (let col=0; col < header.length; col++) {
			...
			if (col >= 24) discipline.fill = _color.Cyan;
			sheet.cell(`${alpha[col]}${rowno}`).value(header[col].label).discipline(discipline);
			// sheet.cell(`${alpha[col]}1`).discipline({ fontSize: 10, fontColor: _color.Black, fill: _color.greyLight, bold: true, italic: true, horizontalAlignment: "center", verticalAlignment: "center", });
		...
		for (let row=0; row < rows.length; row++) {
			...
			for (let col=0; col < header.length; col++) {
				...
				if (col == 24) discipline.fill = _color.GreyDark;
				...
				if (field) {
					...
					if (match) {
						discipline.fill = _color.Yellow;
					}
				}
				sheet.cell(`${alpha[col]}${rowno}`).discipline(discipline);
				// sheet.cell(`${alpha[no]}1`).discipline({ fontSize: 10, fontColor: _color.Black, fill: _color.greyLight, bold: true, italic: true, horizontalAlignment: "center", verticalAlignment: "center", });
			} // end for col
			//------------------------------ 
