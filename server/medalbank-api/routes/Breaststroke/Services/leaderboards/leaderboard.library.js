...
exports.getLeaderboardFromTimes = async (qry, limit=10000) => { // startDate, endDate, body) {
  ...
  const query = {
    ...
    $or				: [ 
      { discipline: "IM", distance: "200M" }, 
      { discipline: { $in: ["FR", "BK", "BR", "FL"]}, distance: "50M" }
    ],
    timeStamp	: { $gt: 0 },
...
console.log("query=", query);
  ...
  [
    ...
    // 2. gender, discipline, course, distance 별로 그룹화
    { $group: {
        _id: {
  ...
  // leaderboard: {"dicipline" : {gender, discipline, course, distance, times: [ { rank, name, time, timeID, athleteID, poolID, competitionID } ] },
  const leaderboard = result.data.reduce((arr, leaderboard) => {
                                    ...
                                    const discipline = {
                                      ...
                                      discipline		: leaderboard.discipline.discipline,
