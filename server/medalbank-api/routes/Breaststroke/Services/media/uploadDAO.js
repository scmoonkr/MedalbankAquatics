...
console.log("1-----> file_meta=", file_meta);
	...
	if (!body.id) {
		body.id = await mongodb.max(mongoCFG.Breaststroke.mediaServer, "id", { category: body.category });
	}
...
async function updateImageMeta(file_meta) {
	...
	const result = await mongodb.updateOne(mongoCFG.Breaststroke.mediaServer, query, file_meta);
	return result.data; 
	} catch(err) {
