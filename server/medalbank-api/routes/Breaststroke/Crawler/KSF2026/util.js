...
exports.getClassCode = (classCd, event) => {
  if (!classCd || classCd.length < 4) return {};
  let school = this.SchoolTable.find(el => el.code == classCd.slice(2, 3)) ?? {};
  if (!school) {
    const sch = this.SchoolTable.find(el => el.label == ageGroup);
    if (sch) {
      school = sch;
    }
  }
