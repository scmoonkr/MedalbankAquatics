...
const global = {}; 
//----- home
// C:\Users\scmoo\OneDrive\5. 메달뱅크\Design
...
// home pc
oneDrive 					= "C:/Users/scmoo/OneDrive"; 
...
function checkSubName(records) {


  let obj = {};
  let old = "";
  for (const record of records) {
    if (!record.name && record.subname) {
      obj[record.subname] = record;
    } else {
...
async function readExcel(filename, sheet) {
	const ck = fs.existsSync(filename);
		...
		console.log("customizing..", rows.length);
		const records = [];
		let no = 1;
		...
		for (const data of rows) {
			const value = {
				...
				array				: data["__EMPTY_7"] 			|| "",
			}
			if (value.name == "end") break;
			if (value.name || value.subname) {
				records.push(value);
				no++
			}
