...
// const _styles 	= [ "접영", "배영", "평영", "자유형", "개인혼영", "혼계영", "혼성계영", "혼성혼계영", "계영", "핀계영", "접영핀", "배영핀", "평영핀", "자유형핀", ];
// const _styles 	= [ "접영", "배영", "평영", "자유형", "개인혼영", "혼계영", "혼성계영", "혼성혼계영", "계영", "핀계영", "계영핀", "혼계영핀", "혼성계영핀", "접영핀", "배영핀", "평영핀", "자유형핀", ];
...
exports.customTimes = (timeORG) => {
	...
	let numbers = [];
	if (typeof timeORG === 'number') {
		if (timeORG > 0 && timeORG < 1) { // 0.000605324074074074
			timeORG = utilDate.convertTimestamp2string(timeORG);
			numbers = timeORG.split(/'|"|-|_|,|\.|:|;/);
			if (numbers.length < 3) numbers = ["00", ...numbers];
		} else if (timeORG > 100) { // 10545
			timeORG = "000000" + timeORG.toString();
			numbers = [timeORG.slice(-6, -4), timeORG.slice(-4, -2), timeORG.slice(-2)];
			timeORG = timeORG.slice(-6, -4) + ":" +  timeORG.slice(-4, -2) + "." + timeORG.slice(-2);
		} else { // 26.95
			timeORG = `00:${timeORG}`;
			numbers = timeORG.split(/'|"|-|_|,|\.|:|;/);
		}
	} else if (timeORG instanceof Date) { // Date
		const minutes = timeORG.getMinutes();
		const seconds = timeORG.getSeconds();
		const milisec = timeORG.getMilliseconds() / 1000;
		numbers = [minutes.toString(), seconds.toString(), milisec.toString()];
		timeORG = `${minutes}:${seconds}.${milisec}`
	} else if (!isNaN(timeORG) && !timeORG.includes(".")) { // 003251
		timeORG = "000000" + timeORG;
		numbers = [timeORG.slice(-6, -4), timeORG.slice(-4, -2), timeORG.slice(-2)];
		timeORG = timeORG.slice(-6, -4) + ":" +  timeORG.slice(-4, -2) + "." + timeORG.slice(-2);
	} else { // string
		// timeORG = timeORG.replace("'", ":").replace('"', ".");	// 00'55\"99
		// timeORG = timeORG.replace(".;", ".").replace(";.", ".").replace(";", ":").replace(';', ".");	// 00'55\"99
		// timeORG = timeORG.replace("-", ":").replace('-', ".");	// 00'55\"99
		// timeORG = timeORG.replace("'", ":").replace("'", ".");	// 00'55\"99
		// timeORG = timeORG.replace('"', ":").replace('"', ".");	// 00'55\"99
		// timeORG = timeORG.replace(",", ":").replace(',', ".");	// 00'55\"99
		// if (!timeORG.includes(".")) timeORG += ".00";
		timeORG = timeORG.replace(".F", ".00").replace('F', ".00").replace("..",".");	// 12.34F
		if (timeORG.split(":").length == 2 && !timeORG.includes(".")) timeORG += ".00"; // 12:4
		if (timeORG.split(".").length == 2 && !timeORG.includes(":")) timeORG = "00:" + timeORG;
		
		const arr = timeORG.split(/'|"|-|_|,|\.|:|;/);
		switch (arr.length) {
			case 1:
				numbers = ["00", "00", ...arr];
				break;
			case 2: // 34.56
				...
				break;
			case 3: // 12:34.56
				timeORG = arr[0] + ":" + arr[1] + "." + arr[2];
				numbers = [...arr];
				break;
		}
	}
	timeORG = "";
	timeORG += `${numbers[0]}:${utilLibrary.leadingZeros(numbers[1],2)}.${(numbers[2] + "00").slice(0, 2)}`;
	if (timeORG.slice(0,3) == "00:") timeORG = timeORG.slice(3);
	if (timeORG.slice(0,2) == "0:") timeORG = timeORG.slice(2);
	if (timeORG.slice(0,1) == "0") timeORG = timeORG.slice(1);
	
	return timeORG;

	const arr = timeORG.split(":");
	let arr1 = arr[arr.length-1].split(".")
	if (arr1.length == 3) { arr[0] = arr1[0]; arr1 = arr1.slice(1); } // 12.34.56
	if (arr1.length == 1) arr1.push("00");

	timeORG = "";
	if (arr.length > 1) timeORG += `${arr[0]}:`;
	timeORG +=  utilLibrary.leadingZeros(arr1[0],2) + "." +(arr1[1] + "00").slice(0, 2); 
	if (timeORG.slice(0,1) == "0") timeORG = timeORG.slice(1);
...
exports.checkTimes = (time) => {
	...
	if (typeof time.time == "number") {
		if (time.time < 1) {
			...
			time.timestamp = this.convertInt2Times(time.time);	
...
exports.checkStatus = (time) => {
	...
	if (time.times && typeof time.times == "string" && ["DQ","DNS","NT","불참","실격","포기","번외"].includes(time.times.toUpperCase()) ) {
		time.note = time.times; // ["DNS","포기","불참"].includes(time.times.toUpperCase()) ? "DNS" : "DQ";
		time.times = "";
	} else if (time.rank && ["DQ","DNS","NT","불참","실격","포기","번외"].includes(time.rank.toString().toUpperCase())) {
	...
	if (time.note) {
		...
		time.note = time.note.replace(/DNS|DQ|NT|번외|포기|실격|불참/gi, "").trim();
	}
	return time;
}
...
exports.checkDiscipline = (time) => {
	...
	// ageGroup에 "남자/여자"
	if ((ageGroup.includes("남") || ageGroup.includes("여")) && !time.gender && !discipline.gender) {
		console.log("\n\n\n남자/여자 ------------------->", time, "\n\n");
		// process.exit();
	}
