...
const {customTimes} = require("../Class/utilTimes");
...
class MemoryDB {
	...
	getCompetition = (competitionID) => {
		console.log("getCompetition=", competitionID, _competitionOBJ);
	...
	*/
	...
	//
	async loadStatics() {
		_statisticsOBJ.times 				= await mongodb.count(mongoCFG.Breaststroke.times, {})
		_statisticsOBJ.athletes 		= await mongodb.count(mongoCFG.Breaststroke.athletes, {})
		_statisticsOBJ.reports			= await mongodb.count(mongoCFG.Breaststroke.reports, {})
		_statisticsOBJ.teams				= _teams.length; // await mongodb.count(mongoCFG.Breaststroke.teams, {})
		_statisticsOBJ.competitions	= _competitions.filter(comp => comp.timeCount > 0).length;
		_statisticsOBJ.stems 				= await mongodb.count(mongoCFG.Breaststroke.stems, {})
		_statisticsOBJ.pools 				= await mongodb.count(mongoCFG.Breaststroke.pools, { competitionCount: { $gt: 0 } })
	...
	async loadConfig() {
		...
		const result = await mongodb.find(mongoCFG.Breaststroke.config, context);
		// 1. stems - _stemOBJ
		_configOBJ = {};
	...
	async loadPools() {
		...
		const result = await mongodb.find(mongoCFG.Breaststroke.pools, context);
	...
	loadUsers = async () => {
		...
		_userOBJ = {};
		const result = await mongodb.find(mongoCFG.Breaststroke.users, context);
		if (result.data.length == 0) return _userOBJ;
		// 1. userID - userID
...
async function getHistory(query, limit=HISTORY_LIMIT) {
	...
	//---------------------------------
	const result = await mongodb.aggregate(mongoCFG.Breaststroke.history, aggregate);
	//---------------------------------
