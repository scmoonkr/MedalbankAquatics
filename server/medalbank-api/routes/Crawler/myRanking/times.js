...
const _year = process.argv.length < 3 ? 2022 : Number(process.argv[2]);
const _seq = process.argv.length < 4 ? 0 : Number(process.argv[3]);

//============================================
(async () => {
let result;

	const query = { year: _year, seq: _seq };
	result = await mongodb.findOne("myCompetitions", query, { _id:0, no:1, seq:1, year:1, cid:1, name:1} );
	if (result.data.year == undefined) return;

	const competition = JSON.parse(JSON.stringify(result.data));
...
async function competitionsPage(page, competition) {
	...
	async function parsingDiscipline(page, category, gender, name, id) {
		...
		// adults, students
		$("#tab1 > div.records_table").each((index, categoryPtr) => {
			// dl sex:남자, age:1그룹, style:자유형, distance:50M
			$(categoryPtr).find("dl.sex").each((index, genderPtr) => {
				// dl age 1그룹
				$(genderPtr).find("dl.age").each((index, ageGroup) => {
					// dl style
