...
(async () => {
  ...
  while (hasNext) {
    ...
    // await page.waitForTimeout(1000); // 안정적 렌더링 대기	
		await delay(1000);
    ...
    // 다음 버튼 클릭
    const nextButton = await frame.$('button[aria-label="Next"], .btn-next, .navigation-next');
    if (nextButton) {
      await nextButton.click();
      index++;
      // await page.waitForTimeout(1200); // 페이지 변경 대기	
			await delay(1200);
    } else {
      hasNext = false;
    }
