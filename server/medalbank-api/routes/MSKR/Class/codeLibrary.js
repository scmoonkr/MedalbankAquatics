...
// 나이에 맞는 그룹 찾기
function findAgeGroup(dob) {
  const age = calculateDob2Age(dob);
  
  // 나이에 맞는 그룹을 찾기
  for (const group of ageGroupTable) {
		// check 학생, 성인 제외
		if (group.ageFrom == 0 || group.ageTo == 100)	continue;
  ...
  if (age < 20) {
    return ageGroupTable.find(g => g.group === '09');
  } else {
    return ageGroupTable.find(g => g.group === '10');
  }
}
exports.getAgeGroupCode = (dob) => {
  ...
  for (const group of this.ageGroupTable) {
	...
	return age <= 20 ? "09" : "10"; // group - 학생: 성인
}
