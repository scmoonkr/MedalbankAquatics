...
exports.indexingNames = (body) => {
	...
	if (body.names) {
		for (const name of nameArr) {
			const norm = utilLibrary.normalizeName(name);
			if (!names.includes(name)) names.push(norm);
		}
	}
	...
	for (const name of nameArr) {
		...
		if (!indexes.includes(name)) indexes.push(norm);
	}
...
exports.normalizeName = (str) => {
	if (str !== undefined && str.length > 0) {
		...
		// Remove all special characters except ','
		str = str.replace(/[^a-zA-Z0-9ㄱ-ㅎㅏ-ㅣ가-힣,+&%\s-]/g, '').trim();
	}
	return str;
}
