...
exports.indexing = async (value, fields) => {
  let indexes = [];

	let arr = fields.split(",");
	arr.forEach((el) => {
		const res = this.normalizeString(value[el]);
...
exports.convertJSON2Contents = (json) => {
    let contents = {
        ...
        type	    : (json.type === undefined || json.type === null ? "" : json.type),
        siteID	    : (json.siteID === undefined || json.siteID === null ? "" : json.siteID),
        user	    : (json.user === undefined || json.user === null ? "" : json.user),
        image	    : (json.image === undefined || json.image === null ? {} : json.image),
        review	    : (json.review === undefined || json.review === null ? {} : json.review),
        authors	    : (json.authors === undefined || json.authors === null ? [] : json.authors),
        titles	    : (json.titles === undefined || json.titles === null ? [] : json.titles),
        category    : (json.category === undefined || json.category === null ? "" : json.category),
        leader	    : json.leader,
        update	    : json.update,
    };

    if (json.authors !== undefined && json.authors !== null && json.authors.length > 0) contents.authors = json.authors;

	contents.tags = []
	let tag = {};
    tagDef.forEach(function (tg) {
		tag = {
			tagno	: tg.tagno,
			ind1   	: tg.ind1,
			ind2   	: tg.ind2,
		...
		if (tg.sfld.length == 0) { // control field
			if (json[`tag${tag.tagno}`] !== undefined) {
				let subfield = {
					sfld:	tg.sfld,
					data:	json[`tag${tag.tagno}`],
				};
				tag.subfield.push(subfield);
				contents.tags.push(tag);
			}
		} else {
			for (let no=0; no<tg.sfld.length; no++) {
				let sfld = tg.sfld.substr(no, 1);
				let tagname = `tag${tag.tagno}${sfld}`;
				if (json[tagname] === undefined) continue;		

				let arr = json[tagname].split('|');
	...
	// 	} else {
	// 		console.log(tag);
	// 		for (let no=0; no<tag.sfld.length; no++) {
	// 			let sfld = tag.sfld.substr(no, 1);
	// 			let tagname = `tag${tag.tagno}${sfld}`;
	// console.log(tagname);
	// 			let arr = json[tagname].split('|');
	// 			arr.forEach(function (data) {
...
exports.field_validation = (body, fields, minFields = 0) => {
	...
	fields.forEach((field) => {
		let ret = Object.keys(body).indexOf(field)
		if (ret < 0 || body[field] === "") notExists.push(field);
	})
	if (notExists.length > 0) {
		return notExists.join(", ") + " not exists or empty !!";
	}
	// PK, update field
	if (minFields > 0 && Object.keys(body).length < minFields) {
		console.log("err");
...
exports.transactionLog = async function log(trCode, result, req) {
	...
	try {
		const value = {
			...
			method		: req.method || "",
			url				: req.url || "",
			userNo		: req.body.accessUserNo || 0,
			params		: req.method.toUpperCase() == "POST" ? req.body : req.params,
			result		: result || "",
			datetime	: req._startTime, // new Date(),
		}
		console.log("transactionLog:", value);
		//----------------------------------------------------------------
		// const result = await mongodb.insertOne(mongoCFG.Medalbank.logs, value);
		//----------------------------------------------------------------
	}
	catch(e) {
		console.log("transactionLog.insertOne.error=" + e);
	}
	//----->
}
