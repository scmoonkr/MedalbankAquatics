...
//-------------------------------------
exports.detailByCommentID = (query, body) => {}
//-------------------------------------
...
exports.detail = (body) => {  // dbType: 'community', dbID: 1
  ...
  if (userID || !isNaN(userID)) aggregate.push(lookupMyReactions);
  aggregate.push(lookupReactions);
  aggregate.push({ $sort: body.sort || { _id: -1 } });
  aggregate.push({ $skip: body.skip || 0 });
  aggregate.push({ $limit: body.limit || max_limit });
  //---------------------------------
...
//-------------------------------------
//-------------------------------------
exports.detailOld = (body) => {  // dbType: 'community', dbID: 1
  console.log("body=",body);
  const userID = Number(body.userID || 0);
  const dbType = body.dbType
  const dbID = Number(body.dbID);
