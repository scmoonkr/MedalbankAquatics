...
class TeamPointsClass {
	...
	async calculateTeamTotalPointsNew() {
		...
		await mongodb.deleteMany(mongoCFG.Medalbank.teamPoints, { competitionID: 0 });
		await mongodb.insertMany(mongoCFG.Medalbank.teamPoints, teamTotalPointArr);
	}
	...
	async buildTimes2TeamStatic(context, competitionID) {
		...
		//-----> insert teamPoints
		await mongodb.insertMany(mongoCFG.Medalbank.teamPoints, [ ...totalArr, ...teamArr ] );
	}
	//--------------------------------------
	//  times -> team statics
	//--------------------------------------
	async buildTimes2TeamPointsNew(times, competitionID) {
		...
		//-----> insert teamPoints
		await mongodb.insertMany(mongoCFG.Medalbank.teamPoints, teamArr );
