...
exports.deleteAthletesNoName = async (body) => {
  ...
  await mongodb.deleteMany(mongoCFG.Medalbank.athletes, query);
}
...
exports.unsetTimesTeamID = async (body) => {
  ...
  const teams = await mongodb.distinct(mongoCFG.Medalbank.teams, "teamID");	
  console.log("teams.teamID.length=", teams.data.length);
  
    const times = await mongodb.distinct(mongoCFG.Medalbank.times, "teamID");	
    ...
    if (teamIDs.length > 0) {
      result = await mongodb.updateManyOP(mongoCFG.Medalbank.times, { teamID: { $in: teamIDs } }, { $unset: { teamID:1}});	
    }
