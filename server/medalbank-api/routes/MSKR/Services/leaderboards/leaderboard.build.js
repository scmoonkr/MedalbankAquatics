...
// process.exit();
  ...
  const aggregate = [
    ...
    { $project: { _id:0, userID:1, teamID:1, poolID:1, competitionID:1, name:1, gender:1, style:1, distance:1, course:1, time:1, timeStamp:1, rank:1, isOfficial:1, isAdult:1 } },
    { $sort: { isMasters:1, isOfficial:1, isAdult:1, gender:1, style:1, course:1, distance:1, timeStamp:1 } }
  ];
  //----------------------------------------------------------------
...
exports.buildLeaderboard = async (body) => {
  ...
  times = this.calculateLeaderboardStatistics(times.data);
	...
	result = await mongodb.insertMany(mongoCFG.Medalbank.teamStatics, leaderboard );
...
exports.calculateLeaderboardStatistics = async (times) => {
  ...
  const grouped = _.groupBy(times, (entry) => `${entry.isAdult??true}-${entry.gender}-${entry.style}-${entry.course}-${entry.distance}`);
...
async function insertLeaderboard(query, times) {
  ...
  //-------------------------------
  await mongodb.insertMany(mongoCFG.Medalbank.leaderboard, times);
  //-------------------------------
}
//===========================================
//  build leaderboard
  ...
  // for (const official of [true, false]) {
    ...
    for (const adult of [true, false]) {
      ...
      for (const gender of ["men", "women"]) {
        //-------------------------------
        for (const style of [ "backstroke", "breaststroke", "butterfly", "freestyle","individualMedley"]) {
          //-------------------------------
