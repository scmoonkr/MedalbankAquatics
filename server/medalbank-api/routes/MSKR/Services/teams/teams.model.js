...
class TeamModel {
	...
	static async saveTeamWithImage(body) {
		...
		if (!body.teamID || body.teamID == 0) {
				...
				value.teamID = await mongodb.max(mongoCFG.Medalbank.teams, "teamID", {});
		} else {
				// 제공된 teamID를 사용
		...
		// 데이터베이스에 업데이트
		// --------------------------------------------------------------
		await mongodb.updateOne(mongoCFG.Medalbank.teams, { teamID: value.teamID }, value);
...
// console.log("\nbody:", body, "\nsort:", sort, "\nlimit:", limit, "\nskip:", skip, "\ndata:", result.data[0]);
		...
		const counts = await mongodb.distinct(mongoCFG.Medalbank.times, "teamID", query );
	...
	static searchRealtime = (async (body) => {
		...
		// const teamTimes = await mongodb.find(mongoCFG.Medalbank.times, context );
...
console.log("competitionIDs=", competitionIDs);
	...
	static async merge(body) {
		...
		//----------------------------------------------------------------
		// return await mongodb.updateMany(mongoCFG.Medalbank.times, query, value);
		//----------------------------------------------------------------
	};
	...
	static async viewOLD(teamID) {
		...
		const aggregate = [
			...
			// { $lookup:{
			// 		from				: mongoCFG.Medalbank.teamStatics,
			// 		localField	: "teamID",
			// 		foreignField: "teamID",
			// 		as					: "statistics"
			// 	}
		...
		const result = await mongodb.aggregate(mongoCFG.Medalbank.teams, aggregate );
		if (result.data.length == 0) return { message: "", data: {} };
...
// console.log("result.data.statistics=", result.data);
  ...
  const projection = { _id:0, teamID:1, name:1, nameKor:1, nameEng:1, names:1, indexes:1 };
	...
	const result = await mongodb.findOne(mongoCFG.Medalbank.teams, query, projection);
  ...
  const resultMerged = await mongodb.find(mongoCFG.Medalbank.teams, context);
  ...
  teamSrc.names = names;
	...
	returnObj = await mongodb.updateOne(mongoCFG.Medalbank.teams, query, teamOBJ);

  //--------------------------------------------------
  //  update teamID teams
  //--------------------------------------------------
  const value = { teamID: teamID };
	returnObj = await mongodb.updateMany(mongoCFG.Medalbank.teams, mergedQuery, value);
	returnObj = await mongodb.updateMany(mongoCFG.Medalbank.leaderboard, mergedQuery, value);
	returnObj = await mongodb.updateMany(mongoCFG.Medalbank.athletes, mergedQuery, value);

  //--------------------------------------------------
  //  remove teams
  //--------------------------------------------------
	returnObj = await mongodb.deleteMany(mongoCFG.Medalbank.teams, mergedQuery);
  ...
  context = {
	...
	returnObj = await mongodb.updateOne(mongoCFG.Medalbank.teams, { teamID    : teamID }, team);
	...
	static async resetAthletesPersonalTeamID() {
		...
		for (const athlete of athletes.data) {
			...
			const time = await mongodb.findOne(mongoCFG.Medalbank.times, query, projection)
			if (time.data.team) {
	...
	static async setAthletesTeamID() {
		...
		let athletes = await mongodb.distinct(mongoCFG.Medalbank.athletes, "team", query)
		...
		for (const teamName of athletes.data) {
			...
			if (name) {
				...
				if (team) {
					...
					const value = { teamID: team.teamID };
					await mongodb.updateMany(mongoCFG.Medalbank.athletes, query, value)
				} else {
					notExists.push(teamName);
				}
			}
	...
	static rankCompetition = (async (body) => {
		...
		//-----> get team points
		let result = await mongodb.findOne(mongoCFG.Medalbank.config, { type: "teamPoints" }, { _id:0, type:0, } );
	...
	static async detail(body) {
		...
		const result = await mongodb.findOne(mongoCFG.Medalbank.teams, query, { _id:0, teamID:1, logo:1, name:1, teamCode:1, sido:1, masters:1, featured:1, adult:1,});
...
console.log("=====>", result);
	...
	console.log("teams.insert.value=", value);
	
	//-----> check exists
	const norm = utilLibrary.normalizeString(value.name);
	const check = await mongodb.findOne(mongoCFG.Medalbank.teams, {indexes: norm}, { _id:0, })
	if (check.data.name) {
	...
	body.teamID = await mongodb.max(mongoCFG.Medalbank.teams, "teamID", {});
	const teamOBJ = this.indexing(body);

	console.log("teams.insert.teamOBJ.", teamOBJ);
	const returnObj = {}; // await mongodb.insertOne(mongoCFG.Medalbank.teams, teamOBJ);
	...
	static async update(body) {
		...
		const value = Customizing.field(body);
		if (!value.teamID) value.teamID = await mongodb.max(mongoCFG.Medalbank.teams, "teamID", {});
		const query = { teamID: value.teamID };
		// delete value.teamID;

		const result = await mongodb.findOne(mongoCFG.Medalbank.teams, query);
	...
	console.log("update=", query, value);
		//----------------------------------------------------------------
		return await mongodb.updateOne(mongoCFG.Medalbank.teams, query, value);
		//----------------------------------------------------------------
	}
  ...
  static async updateDelete(teamID, userID) {
    const query = { teamID: Number(teamID), userID: Number(userID) };
      ...
      deleted : new Date(),
		...
		//----------------------------------------------------------------
		returnObj = await mongodb.updateOne(mongoCFG.Medalbank.teams, query, value);
		//----------------------------------------------------------------
	...
	static async delete(teamID) {
		try {
			...
			//----------------------------------------------------------------
			return await mongodb.deleteOne(mongoCFG.Medalbank.teams, query);
			//----------------------------------------------------------------
		} catch (e) {
