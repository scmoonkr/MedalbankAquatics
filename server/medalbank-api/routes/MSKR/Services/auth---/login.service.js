...
class LoginService {
	...
	static async signup(body){
		try{
			...
			return await Model.insert(signup);
		}catch(err){
			console.log(err);
		}
	}
	...
	static async setPassword(body){
		try{
      console.log("auth.services.login.setPassword.body.", body);
			if (!body.password			) return utilError.errorMSG("Service","login", "setPassword", "password 없음 !!");
			if (!body.registrationNo) return utilError.errorMSG("Service","login", "setPassword", "registrationNo 없음 !!");

			return await Model.setPassword(body);
		}catch(err){
      return utilError.errorMSG("Service","login", "setPassword", "delete.catch." + err);
		}
	}
