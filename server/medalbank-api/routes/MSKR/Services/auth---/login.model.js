...
class LoginModel {
  ...
  static async login(body){
    ...
    try {
      ...
      const projection = { _id:0, email:1, name:1, nickname:1, password:1, userID:1, dob:1, gender:1, ageGroupCode:1, athleteID:1, role:1, };
      //----------------------------------------------------------------
    ...
    try {
      returnObj.token = await utilJWT.sign({userID: result.data.userID, nickname: result.data.nickname, email: result.data.email, role: result.data.role, });	 //body Json web token으로 변환
    } catch(e) {
...
//####################################################################
  ...
  static async setPassword(body){
    console.log("auth.model.login.setPassword.body.", body);
    const returnObj = { message: "", data: {}};
    const query 	  =	{ registrationNo: body.registrationNo }
    try {
      const projection 	= { _id:0, userID:1 }
      const result  = await mongodb.findOne(mongoCFG.Medalbank.users, query, projection);  
      console.log("find.", result.data);       // mongodb에서 가져온 userinfo를 newpassword로 update
      if (!result.data.userID) return utilError.errorMSG("Model","users", "setPassword", "email없음");
    } catch(e) {
      return utilError.errorMSG("Model","users", "setPassword", "findOne.catch." + e);
    }
  ...
  static async lostPasswordEmail(nickname){
    ...
    let message = mongoCFG.mskServer + "/users/setPassword?token=" + userInfo._id ;
