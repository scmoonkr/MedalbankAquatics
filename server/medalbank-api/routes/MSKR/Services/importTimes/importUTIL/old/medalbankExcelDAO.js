...
const TimesDAO 		= require('../../database/times/customizing');
...
const value = {
	...
	year						: data.year 						|| 0, // 년도
	order						: data.order 						|| 0, // 회차
	hosts						: data.hosts 						|| "", // 주최
	managers				: data.managers 				|| "", // 주관
	supporters			: data.supporters 			|| "", // 후원
	sponsers				: data.sponsers 				|| "", // 협찬
	poolID					: data.poolID 					|| 0,
	pool						: data.pool 						|| "", // 수영장
	sido						: data.sido 						|| "", // 시도
	address					: data.address 					|| "", // 주소
	email						: data.email 						|| "", // 이메일
	website					: data.website 					|| "", // 홈페이지
	phone						: data.phone 						|| "", // 전화
	dateStart				: data.dateStart 				|| "", // 대회 시작일자
	dateEnd					: data.dateEnd 					|| "", // 대회 종료일자
	dateStartSignup	: data.dateStartSignup 	|| "", // 등록 시작일자
	dateEndSignup		: data.dateEndSignup 		|| "", // 등록 종료일자
	dateStartReceipt: data.dateStartReceipt	|| "", // 접수 시작일자
	dateEndReceipt	: data.dateEndReceipt 	|| "", // 접수 종료일자
	ageGroup				: data.ageGroup 				|| [], // times build후 생성
	course					: data.course 					|| "", // 코스
	measured				: data.measured 				|| true, // 자동계측
	grade						: data.grade 						|| "", // 급
	entryLimits			: data.entryLimits 			|| 0, // 접수정원
	target					: data.target 					|| "", // 참가대상
	tags						: data.tags 						|| [], // []
	featuredImage		: data.featuredImage 		|| "", // 대표이미지
	outlines				: data.outlines 				|| "", // 대회요강
	timesFile				: data.timesFile 				|| "", // 기록파일
	timesExcel			: data.timesExcel 			|| "", // 기록 엑셀파일
	images					: data.images 					|| "", // []
	note						: data.note 						|| "",
	masters					: data.masters 					|| true,
...
exports.checkDuplicate = (times) => {
	const timeOBJ = {};
	for (const time of times) {
		// const key = `${time.competitionID}-${time.gender}-${time.ageGroup}-${time.style}-${time.distance}-${time.name}-${time.team}-${time.times}`;
		const key = `${time.competitionID}-${time.gender}-${time.style}-${time.distance}-${time.name}-${time.team}-${time.times}`;
		if (!timeOBJ[key]) timeOBJ[key] = [];
		timeOBJ[key].push(`${time.name}, ${time.team}, ${time.times}, ${time.ageGroup}, ${time.gender}, ${time.style}, ${time.distance}`);
	}
	...
	if (timeArr.length > 0) {
		...
		console.log("2-----> check duplicate...OK");
	}
...
exports.readTimesFromExcelORG = async (competition, path, filename, timeID) => {
	...
	const timeArr = await excel.read_excel_org(`${path}/${filename}`);

	let times = [];
	if (timeArr[0]["선수"]) { // complete: 성태가 작업한것
		// console.log("111~~~~~~~~~");
		console.log("customCompleteTimes", timeArr.length);
		times = formatCompleteTimes(timeArr, competition);
	} else {
		times = formatTimes(timeArr);
	}

	times = customizingTimesORG(times, competition);
	console.log("-----> readTimesExcel", times.length);
	return times;
}
...
function customizingTimesORG(timeArr, competition) {
    ...
    for (const row of timeArr) {
      ...
      if (_headerTime.includes(time.times) || time.name && _headerName.includes(time.name.replace(/ /gi, ''))) continue;
  
      //-----> 선수명, 팀명이 없으면
      const ckName = time.name && ! _headerName.includes(time.name.replace(/ /gi, ''));
			const ckTeam = time.team && _headerTeam.includes(time.team.toString().replace(/ /gi, ''));
      if (! ckName && ckTeam) continue;
			if (!time.name && !time.team && !time.time) continue;
      ...
      time.datetime				= ! time.datetime ? competition.dateStart : time.datetime;
			...
			if (time.style && !Util.styleTable.find(style => time.style == style.style)) {
				...
				if (ck) {
					if (!time.distance) {
						const distance = time.style.replace(ck.label, '').trim();
      ...
      time.round = mskCFG.getRoundKor2Eng(time.round);
			...
			//-----> check adult
			if (!time.adult) {
				time.adult = Util.checkAdult(time);
			}
      ...
      if (!time.individual && !time.name) time.name = time.team;
			...
			// if (time.sheet) {
			// 	if (time.sheet.includes("성인") ) {
			// 		time.adult = true;
			// 	}
			// 	if (time.sheet.includes("학생") ) {
			// 		time.adult = false;
			// 	}
			// 	if (time.sheet.includes("개인") ) {
			// 		time.individual = true;
			// 	}
			// 	if (time.sheet.includes("단체") ) {
			// 		time.individual = false;
			// 	}
			// }

      //----------------------------------
      // time = customizingTimes(time, _competition);
  
      //-----> build index
      // time.indexes = Util.buildIndex(time);
  
      //----------------------------------
      //----------------------------------
			if (time.adult) time.adult = "학생,junior,Junior,JUNIOR".indexOf(time.adult) < 0 ? true : false;
      ...
      if (!message.times.includes(time.times) ) 								message.times.push(time.times);
      ...
      // if (time.times == "01.00") { console.log("~~~~~~~", time); process.exit(); }
    } // end for
