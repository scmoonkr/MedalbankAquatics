...
exports.check = async (body) => {
  ...
  console.log("af checkExistsTimes", times.length);
	...
	console.log("~~~~~~~~~~~~~~~~~ medalbank.teams=", teams);
	...
	if (returnObj.data.length > 0) {
		...
		result = await mongodb.deleteMany(mongoCFG.Medalbank.timesImport, query);
		result = await mongodb.insertMany(mongoCFG.Medalbank.timesImport, returnObj.data);
	...
	// 	query = { competitionID: competition.competitionID };
	// 	result = await mongodb.deleteMany(mongoCFG.Medalbank.athletesImport, query);
	// 	result = await mongodb.insertMany(mongoCFG.Medalbank.athletesImport, medalbank.athletes);
	// }
	//-----------------------------------------
