...
exports.buildRankings = async (timeArr=[]) => {
	...
	// try {
		...
		// 1. check rankings
		//--------------------------------------------
		// let result = await mongodb.find(mongoCFG.Medalbank.leaderboard, { query: {},});
		...
		if (times.length > 0) {
			//-----------------------------------------------------
			result = await mongodb.deleteMany(mongoCFG.Medalbank.leaderboard, {});
			result = await mongodb.insertMany(mongoCFG.Medalbank.leaderboard, times);
			//-----------------------------------------------------
