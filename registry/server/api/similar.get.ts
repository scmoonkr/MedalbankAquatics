...
// 같은 종목: 동일 성별·영법·거리·코스에서 선수별 PB → diff 순(동률 시 datetime 오름차순) 10개, 본인 제외
// 타 영법:  성별 무관, 현재 종목 제외 전 종목에서 선수별 PB → diff 순(동률 시 datetime 오름차순) 10개
...
export default defineEventHandler(async (event) => {
  ...
  const athlete  = String(q.athlete  || '')    // 본인 이름 (같은 종목에서 제외)
  ...
  const sameMatch: Record<string, any> = {
    gender: dbGender, discipline: stroke, distance: distStr, course, waPoints: { $gt: 0 },
  }
  if (athlete) sameMatch.name = { $ne: athlete }   // 본인 제외

  // ── 같은 종목: 선수별 PB(min timeStamp) → diff 순, 동률 시 datetime 오름차순 ──
  const sameDocs = await coll.aggregate([
    ...
    { $sort: { diff: 1, datetime: 1 } },                      // diff 동률 시 오래된 기록 우선
    { $limit: 10 },
  ]).toArray()
  ...
  for (const [g, strokes] of Object.entries(BASE_TIMES[course] ?? {})) {
    ...
    for (const [s, dists] of Object.entries(strokes)) {
      for (const d of Object.keys(dists)) {
        const dist = Number(d)
        const ds   = `${dist}M`
        if (gDb === dbGender && s === stroke && ds === distStr) continue  // 현재 종목 제외
  ...
  // 전체 합산 후 diff 순(동률 시 datetime 오름차순) 상위 10개
  const otherAll: any[] = []
  for (const docs of otherResults) otherAll.push(...docs)
  otherAll.sort((a, b) => a.diff - b.diff || (a.datetime < b.datetime ? -1 : a.datetime > b.datetime ? 1 : 0))
  ...
  const fmt = (d: any, s: string, dist: number) => ({
    ...
    time:     d.time     || '',
    athlete:  d.name     || '',
    date:     d.datetime ? String(d.datetime).slice(0, 10) : '',   // yyyy-MM-dd
    points:   d.waPoints ?? 0,
  })
