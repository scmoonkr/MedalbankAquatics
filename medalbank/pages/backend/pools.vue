<template>
  <div class="pl-root">
    <div class="be-page-head">
      <div>
        <div class="be-page-title">Pools</div>
        <div class="be-page-sub">pools collection · {{ rows.length }} entries</div>
      </div>
    </div>

    <!-- Filters -->
    <div class="be-filters">
      <input v-model="f.name" class="be-search" placeholder="수영장명 검색…" @keyup.enter="refresh" />
      <select v-model="f.course" class="be-sel" @change="refresh">
        <option value="">코스 전체</option>
        <option v-for="c in courseTable" :key="c.value" :value="c.value">{{ c.label }}</option>
      </select>
      <select v-model="f.sido" class="be-sel" @change="refresh">
        <option value="">시도 전국</option>
    ...
    <div class="pl-body">
      <div class="pl-content">
        ...
        <div v-else class="be-empty">{{ loading ? '불러오는 중…' : '일치하는 수영장이 없습니다.' }}</div>
      </div>
...
// ── 필터 ──────────────────────────────────────────────────────────
const f       = reactive({ name: '', sido: '', course: '' })
const loading = ref(false)

function resetFilters() { Object.assign(f, { name: '', sido: '', course: '' }); refresh() }
...
async function refresh() {
  ...
  try {
    ...
    if (f.sido)   params.sido   = f.sido
    if (f.course) params.course = f.course
    rows.value = await $fetch<PoolRow[]>('/api/backend/pools', { params })
  } finally {
    loading.value = false
  }
}
...
async function deleteRow() {
  if (!panel.id) return
  if (!confirm(`'${panel.form.pool}' 삭제 하시겠습니까?`)) return
  try {
    await $fetch(`/api/backend/pools/${panel.id}`, { method: 'DELETE' })
    closePanel()
    await refresh()
  } catch (e: any) {
...
.c-tag { display: inline-block; font-size: 10.5px; font-weight: 700; padding: 2px 7px; border-radius: 3px; letter-spacing: 0.06em; background: #e0f2fe; color: #075985; }
