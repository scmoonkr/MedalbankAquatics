        ...
        <template v-else-if="scoringReady">
          ...
          <template v-else>
            ...
            <table class="modal-age-table">
              <thead><tr><th>연령 그룹</th><th>환산 계수</th><th>등가 시간</th></tr></thead>
              <tbody>
      ...
      <section class="modal-block">
        ...
        <div v-else class="modal-pace">
          <div class="pace-paces">
            <div v-for="d in [50, 100, 200, 400]" :key="d" class="pace-cell" :class="{ current: d === state.distance }">
...
const RECORD_TYPES = [
  ...
  { code: 'AR',  full: '아시아기록' },
  { code: 'KR',  full: '한국기록' },
  { code: 'WMR', full: '세계마스터즈기록' },
  { code: 'KMR', full: '한국마스터즈기록' },
  { code: 'ER',  full: '인핸스드게임기록' },
]
...
function goReport() { openSubmitModal?.(submitInitialData.value) }

// ── State: query mode에서는 route.query 기반, ID mode에서는 기본값 후 onMounted에서 채움 ──
const state = reactive({
  gender:   timeID ? 'M'  : (route.query.gender  as string) || 'M',
  stroke:   timeID ? 'FR' : (route.query.stroke  as string) || 'FR',
  distance: timeID ? 50   : Number(route.query.distance)    || 50,
  course:   'LCM',
  timeSec:  null as number | null,
  dob:      timeID ? null : ((route.query.dob as string) || null) as string | null,
})

const attribution = reactive({
  athlete:  timeID ? '' : (route.query.athlete  as string) || '',
  datetime: timeID ? '' : (route.query.datetime as string) || '',
  meet:     timeID ? '' : (route.query.meet     as string) || '',
})

const isDirty        = ref(false)
const timeInputRaw   = ref(timeID ? '' : (route.query.time as string) || '')
...
const h1Parts = computed(() => {
  ...
  const idx = t.indexOf(':')
  return { prefix: t.slice(0, idx).padStart(2, '0') + ':', suffix: t.slice(idx + 1) }
})
...
function onTimeChange(e: Event) {
  ...
  if (!raw.trim()) { state.timeSec = null; timeInputError.value = ''; isDirty.value = true; return }
  const normalized = normalizeTimeInput(raw)
  if (!isValidTime(normalized)) { timeInputError.value = '올바른 기록 형식이 아닙니다 (예: 01:23.45)'; state.timeSec = null; return }
  timeInputError.value = ''
  const parsed = S().parseTime(normalized)
  if (parsed != null && parsed > 0) { state.timeSec = parsed; timeInputRaw.value = normTime(S().formatTime(parsed)) }
  else { state.timeSec = null }
  isDirty.value = true
}
...
function creditStr(r: any): string { return [r.holder, r.nation, r.year, r.ageGroup].filter(Boolean).join(' · ') }
...
function timeForPts(target: number): number | null {
  return scoringReady.value ? S().timeForPoints(state.gender, state.stroke, state.distance, target) : null
}
...
const equivGrid = computed<[string, number][][] | null>(() => scoringReady.value ? S().EQUIV_GRID : null)
...
const ageData = computed(() => {
  ...
  const openTime  = state.timeSec * (myFactor / factors['25-29'])
  const openPts   = S().calcPoints(state.gender, state.stroke, state.distance, openTime)
  const rows = Object.entries(factors).map(([g, fY]) => ({
    group: g, factor: (myFactor / fY).toFixed(4),
    adjTime: state.timeSec! * (myFactor / fY), isMine: g === myGroup,
  }))
...
interface SimilarRecord { gender: string; stroke: string; distance: number; course: string; athlete: string; time: string; date: string; points: number }
interface SimilarPair   { same: SimilarRecord[]; other: SimilarRecord[] }
interface SimilarResponse { oldest: SimilarPair; recent: SimilarPair; sameYear: SimilarPair }
...
async function fetchSimilar() {
  ...
  try {
    ...
    const params = new URLSearchParams({ gender: state.gender, stroke: state.stroke, distance: String(state.distance), course: state.course, pts: String(waPoints.value) })
    ...
    if (raw?.oldest) {
      ...
      similarData.value = { oldest: { same: raw.same, other: raw.other }, recent: { same: raw.same, other: raw.other }, sameYear: { same: [], other: [] } }
    } else {
      similarData.value = null
    }
  } catch { similarData.value = null }
  finally  { similarPending.value = false }
}

watch([() => state.gender, () => state.stroke, () => state.distance, () => state.timeSec, scoringReady, () => attribution.datetime, () => attribution.athlete], fetchSimilar)
...
function downloadPdf() {
  ...
  if (shellClone) {
    ;['.tv-actions', '.tv-copy-toast'].forEach(sel => { const el = shellClone.querySelector(sel) as HTMLElement | null; if (el) el.style.display = 'none' })
  }
  ...
  if (!win) return
  win.document.write(`<!DOCTYPE html><html lang="ko"><head><meta charset="utf-8"><title>${title}</title><style>${styles}@page{size:A4 portrait;margin:6mm}html,body{margin:0;padding:0;background:#fff}.tv-pdf-shell{padding:28px 44px;width:100%}.page-body{padding:0!important;margin:0!important}.stub-shell{padding:0 0 28px!important;border-bottom:1px solid var(--rule)}.stub-shell h1{font-size:36px!important;margin-bottom:16px!important}.tv-pdf-credit{display:block;margin-top:20px;padding-top:12px;border-top:1px solid #ddd;font-family:var(--sans);font-size:9px;letter-spacing:.18em;text-transform:uppercase;color:#aaa}</style></head><body><div class="tv-pdf-shell">${shellClone ? shellClone.outerHTML : ''}${clone.outerHTML}<div class="tv-pdf-credit">메달뱅크 · medalbankaquatics.com · ${new Date().toISOString().slice(0, 10)}</div></div></body></html>`)
...
function parseRecTime(t: string): number | null {
  if (!t) return null
...
async function injectCanonOverlay() {
  try {
    ...
    canonLoaded.value++
  } catch (e) { console.error('[time] canon inject failed', e) }
}

interface TimeRecord { timeID: number; gender: string; stroke: string; distance: number; course: string; time: string; athlete: string; datetime: string; meet: string }

onMounted(async () => {
  if (timeID) {
    // ── ID 모드: DB에서 레코드 로드 ─────────────────────────────
    const [_, rec] = await Promise.all([
      loadScript('/canon/js/scoring.js'),
      $fetch<TimeRecord>(`/api/time/${timeID}`).catch(() => null),
    ])
    if (rec) {
      state.gender   = rec.gender   || 'M'
      state.stroke   = rec.stroke   || 'FR'
      state.distance = rec.distance || 50
      state.course   = rec.course   || 'LCM'
      attribution.athlete  = rec.athlete  || ''
      attribution.datetime = rec.datetime || ''
      attribution.meet     = rec.meet     || ''
      const parsed = parseRecTime(rec.time)
      if (parsed && parsed > 0) { state.timeSec = parsed; timeInputRaw.value = rec.time }
    }
    scoringReady.value = true
  } else {
    // ── Query 모드: route.query에서 로드 ────────────────────────
    await loadScript('/canon/js/scoring.js')
    scoringReady.value = true
    const raw = route.query.time as string
    if (raw) {
      const parsed = S().parseTime(raw)
      if (parsed && parsed > 0) { state.timeSec = parsed; timeInputRaw.value = normTime(S().formatTime(parsed)) }
    }
  }
...
.stub-shell { padding-left: 0; padding-right: 0; }
.tv-shell-inner { max-width: 1200px; margin: 0 auto; padding: 0 var(--pad-x); }
.tv-disclaimer { font-family: var(--serif-ko); font-size: 12px; line-height: 1.8; color: var(--fg-mute); max-width: 760px; }
.tv-actions { display: flex; flex-wrap: wrap; gap: 10px; margin-top: 28px; }
.tv-action-btn { display: inline-flex; align-items: center; gap: 6px; padding: 8px 18px; border: 1px solid var(--line); background: transparent; font-family: var(--sans); font-size: 12.5px; letter-spacing: 0.06em; color: var(--fg-dim); cursor: pointer; transition: background 0.15s, color 0.15s, border-color 0.15s; -webkit-tap-highlight-color: transparent; touch-action: manipulation; }
.tv-action-btn:hover { background: var(--bg-soft); color: var(--fg); border-color: var(--fg-mute); }
.tv-action-icon { font-size: 13px; line-height: 1; }
.tv-copy-toast { margin-top: 10px; font-family: var(--mono); font-size: 11.5px; color: var(--fg-mute); letter-spacing: 0.02em; }
.modal-time-input.input-error { border-color: #e05; outline-color: #e05; }
.modal-input-error { font-family: var(--sans); font-size: 11.5px; color: #e05; letter-spacing: 0.02em; margin-top: 4px; }
.pts-diff { display: block; font-size: 10px; color: var(--fg-mute); letter-spacing: 0; margin-top: 1px; }
.sim-set-head { font-family: var(--sans); font-size: 11px; letter-spacing: 0.04em; color: var(--fg-mute); padding: 20px 0 8px; border-top: 1px solid var(--line); margin-top: 12px; display: flex; align-items: baseline; flex-wrap: wrap; gap: 0 4px; }
.sim-set-head:first-child { margin-top: 0; border-top: none; padding-top: 0; }
.sim-set-group { font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; color: var(--fg-dim); }
.sim-set-sep { color: var(--fg-faint); font-weight: 400; }
.sim-set-label { font-weight: 400; color: var(--fg-mute); }
.sim-table-wrap { overflow-x: auto; }
.modal-similar-table { white-space: nowrap; }
.modal-similar-table td, .modal-similar-table th { white-space: nowrap; }
.c-gap  { width: 52px; text-align: right; }
.gap    { font-family: var(--mono); font-size: 12px; color: var(--fg-mute); text-align: right; }
.empty-row td { color: var(--fg-faint); font-size: 12px; padding: 10px 0; text-align: center; }
.tv-footnote-body { font-family: var(--serif-ko); font-size: 13.5px; line-height: 1.8; color: var(--fg-dim); max-width: 760px; }
.tv-footnote-body p { margin: 0 0 12px; }
.tv-footnote-body p:last-child { margin-bottom: 0; }
.tv-footnote-date { font-size: 12px; color: var(--fg-mute); margin-top: 16px !important; padding-top: 12px; border-top: 1px solid var(--line); }
.tv-footnote-wr { font-family: var(--mono); font-size: 11px; color: var(--fg-faint); letter-spacing: 0.02em; }
</style>
